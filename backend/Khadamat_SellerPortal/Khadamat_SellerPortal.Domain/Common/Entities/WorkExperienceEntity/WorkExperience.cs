...
namespace Khadamat_SellerPortal.Domain.Common.Entities.WorkExperienceEntity
{
    /// <summary>
    ...
    public class WorkExperience : Entity
    {
        ...
        /// </returns>
        public ErrorOr<Success> AddCertificate(string filePath, string description, out Func<int> certificateIdProvider)
        {
            if (_certificates.Any(c => c.CachedFilePath == filePath))
            {
                certificateIdProvider = () => -1;
                return Error.Conflict("WorkExperience.CertificateExist", "The provided path already have a certificate");
            }
            var certificate = new Certificate(filePath, description, fileId: 0, workExperienceId: Id, educationId: null);
            _certificates.Add(certificate);
            certificateIdProvider = () => certificate.Id;
            return Result.Success;
        }
        ...
        /// or an error if no certificate with the specified ID was found.
        /// </returns>
        public ErrorOr<Success> UpdateCertificate(int certificationId, string filePath, string description, int fileId)
        {
            var certification = _certificates.Find(c => c.Id == certificationId);
            if (certification == null)
            {
            ...
            return certification.UpdateCertificate(filePath, description, fileId);
        }
